...
function searchCourses(params) {
    ...
    if (params.query) {
        ...
        results = results.filter(course => course.title.toLowerCase().includes(query) ||
            course.description.toLowerCase().includes(query) ||
            course.instructor.toLowerCase().includes(query));
    }
    if (params.subject) {
        results = results.filter(course => course.subject.toLowerCase() === params.subject.toLowerCase());
    }
    if (params.level) {
        results = results.filter(course => course.level.toLowerCase() === params.level.toLowerCase());
    }
    if (params.duration) {
        results = results.filter(course => course.duration <= params.duration);
    }
    if (params.provider) {
        results = results.filter(course => course.provider.toLowerCase() === params.provider.toLowerCase());
    }
    return results;
}
